...
export default function KanbanBoardClient({ leads, isLoading, onLeadUpdate }: KanbanBoardClientProps) {
  ...
  const handleMove = async (event: KanbanMoveEvent) => {
    ...
    // Find the lead being moved and its original column
    ...
    // Optimistic update
    const activeItems = [...columns[activeColumnKey]];
    const overItems = newStatus in columns ? [...columns[newStatus]] : [];
    
    const activeIndex = activeItems.findIndex(l => l.id === leadId);
    const [movedLead] = activeItems.splice(activeIndex, 1);
    
    const newColumns = {
        ...columns,
        [activeColumnKey]: activeItems,
        [newStatus]: [...overItems, { ...movedLead, status: newStatus }]
    };
    setColumns(newColumns);
    ...
    try {
      ...
      // Optionally trigger a full refetch to ensure data consistency
      onLeadUpdate();
    } catch (error: any) {
      toast({
        variant: 'destructive',
  ...
  return (
    <ScrollArea className="w-full whitespace-nowrap rounded-lg">
        <Kanban value={columns} onValueChange={setColumns} onMove={handleMove} getItemValue={(item) => item.id}>
            <KanbanBoard className="p-1">
            {Object.entries(columns).map(([status, statusLeads]) => (
                <KanbanColumn key={status} value={status} className="w-[300px] flex-shrink-0">
                    <div className={cn(
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            <Badge variant="secondary" className='rounded-full px-1.5 text-xs h-5'>{statusLeads.length}</Badge>
                        </div>
                    </div>
                <KanbanColumnContent value={status} className="h-full min-h-[100px] p-2 rounded-b-md bg-muted/50">
